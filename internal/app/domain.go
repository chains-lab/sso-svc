...
import (
	...
	"github.com/hs-zavet/sso-oauth/internal/domain/ape"
	models2 "github.com/hs-zavet/sso-oauth/internal/domain/models"
...
type Domain interface {
	SessionCreate(ctx context.Context, session models2.Session) error
	...
	SessionRefresh(ctx context.Context, session models2.Session, subTypeID *uuid.UUID, role identity.IdnType, IP, client, curToken string) (*string, *string, error)
...
func (d *domain) SessionCreate(ctx context.Context, session models2.Session) error {
...
func (d *domain) SessionRefresh(ctx context.Context, session models2.Session, subTypeID *uuid.UUID, role identity.IdnType, IP, client, curToken string) (*string, *string, error) {
...
func (d *domain) Login(ctx context.Context, role identity.IdnType, subTypeID *uuid.UUID, email, client, IP string) (*string, *string, error) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			account, err = d.AccountCreate(ctx, models2.Account{
				ID:        uuid.New(),
				Email:     email,
				Role:      role,
				UpdatedAt: time.Now(),
	...
	err = d.Infra.Data.SQL.Sessions.New().Insert(ctx, models2.Session{
		ID:        sessionID,
